...
P20 <- .Le_Organiza(arquivoPdf_P20)
P36 <-.Le_Organiza(arquivoPdf_P36)
P48 <- .Le_Organiza(arquivoPdf_P48)
...
Relatorio_corpus <- VCorpus(VectorSource(P20$text))
content(Relatorio_corpus[[15]])
...
##removeWords(text, stopwords("pt"))
new_stops <- c("plataforma", "investigação", stopwords("pt"))
...
.Clean_corpus <- function(corpus) {
  ...
  # Add more stopwords
  corpus <- tm_map(corpus, removeWords, words = new_stops)
  # Strip whitespace
  corpus <- tm_map(corpus, stripWhitespace)
  return(corpus)
}

clean_P20 <- .Clean_corpus(Relatorio_corpus)

##DTM 
.DTM <- function(clean_corp){
corp_dtm <- DocumentTermMatrix(clean_corp)
# Convert dtm to a matrix
dtm_corp_m <- as.matrix (corp_dtm)
return(dtm_corp_m)
}

DTM_P20_m <- .DTM(clean_P20)

.TDM <- function(clean_corp){
# Create a term-document matrix from the corpus
corp_tdm <- TermDocumentMatrix(clean_corp)
# Convert tdm to a matrix
tdm_corp_m <- as.matrix(corp_tdm)
return(tdm_corp_m)
}

TDM_P20_m <- .TDM(clean_P20)

# Review a portion of the matrix
TDM_P20_m[F22[1:2],]


freq <- colSums(DTM_P20_m)

# Create bi_words
bi_words <- names(freq)

str_subset(bi_words,"^falhas")


names(P20)[1] <- "doc_id"


.Bag_vetor <- function(pdf) {
    ...
    myCorpus = tm_map(mycorpus, bracketX)
    myCorpus = tm_map(myCorpus, removeWords,
                      c(stopwords("pt"), "thy", "thou", "thee", "the", "and", "but"))
    ...
    Result <- sort(rowSums(m), decreasing = TRUE)
...
## FUNCAO que agrega todas as funções
 ...
 .Analise_Completa_Documento <- function(texto,Tipo_Acidente) {


 tb <- .Analise_fatores_tb(texto, Tipo_Acidente)
...
.Analise_Completa_Documento(P20, "Incendio")
...
.Analise_Completa_Documento(P36, "Naufragio")
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
 ...
 .Analise_Completa_Documento(P48, "Alagamento")

tb_P48 <-.Analise_fatores_tb(P48, "Alagamento")
...
data4 <- bind_rows(tb_P36, tb_P48, tb_P20) 

data4 <- data4 %>%
select(Fator, Dimensao, Tipo_Acidente)%>%
    group_by(Fator, Dimensao) %>%
   summarise(count = n())
names(data4)  <- c( "group","individual", "value")
...
select(Fator, Dimensao, Tipo_Acidente)%>%
    group_by(Fator, Tipo_Acidente) %>%
   summarise(count = n())
names(data5)  <- c( "group","individual", "value")
...
ggplot(data5, aes(fill=Tipo_Acidente, y=Fator, x=count)) + 
    geom_bar(position="fill", stat="identity") + ggtitle("Fatores humanos por relatório de acidente")
